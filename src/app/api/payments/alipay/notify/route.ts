...
/** Alipay async payment notification. Verifies the signature, then fulfils the order
 *  by outTradeNo (idempotent — safe under gateway retries). */
export async function POST(req: Request) {
  return route(async () => {
    const n = await parsePaymentNotification("alipay", req);
